 ...
 * 支持直接调用 AI API 或通过后端 /api/ai-analyze 代理（推荐，避免 CORS 限制）。
 */
...
/**
 * 通过后端代理调用 AI API，避免浏览器端 CORS 限制。
 *
 * @param {string} apiUrl - AI API 端点 URL。
 * @param {string} apiKey - API Key。
 * @param {!Object} payload - 要发送的请求体。
 * @return {!Promise<!Object>} API 响应数据。
 */
async function _callAIProxy_(apiUrl, apiKey, payload) {
  ...
  if (!result.success) {
    throw new Error(result.msg || '代理请求返回失败');
  }
  return result.data;
}
...
async function testConnection() {
  ...
  try {
    ...
    const data = await _callAIProxy_(apiUrl, apiKey, testRequestBody);
...
async function analyzeWithAI(simulationParams = null) {
  ...
  try {
    ...
    const data = await _callAIProxy_(apiUrl, apiKey, requestBody);
...
function bindAIEvents_() {
  ...
  if (AIState.elements.analyzeBtn) {
    AIState.elements.analyzeBtn.addEventListener(
        'click',
        () => analyzeWithAI(AIState.simulationParams),
    );
  }
}
 ...
 * @param {?Object=} params - 仿真参数（可选）。
 */
export function setSimulationData(data, params = null) {
  AIState.simulationData = data;
  AIState.simulationParams = params;
  if (AIState.elements.analyzeBtn) {
